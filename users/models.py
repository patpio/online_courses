...
class Account(AbstractBaseUser, PermissionsMixin):
    email = EmailField(verbose_name='email', max_length=30, unique=True)
    fullname = CharField(max_length=30, unique=True)
    is_instructor = BooleanField(default=False)
    date_join = DateTimeField(verbose_name='date join', auto_now_add=True)
    last_login = DateTimeField(verbose_name='last join', auto_now_add=True)
